...
import CaLoadJobs from '../components/CaLoadJobs';
...
function Hcampus() {
  ...
  const setUser = async ()=>{
      if(!localStorage.getItem("findoutc")){
        navigate('/login');
      }
      else{
        setCurrentUser(await JSON.parse(localStorage.getItem("findoutc")));
        setIsLoaded(true);
        setTimeout(async()=>{
  ...
  return (
    ...
    <CaLoadJobs></CaLoadJobs>
 <ToastContainer/>
  </HcompanyWrapper>
  )
}

const HcompanyWrapper = stylerr.div`
  width: 100vw;
